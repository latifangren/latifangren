#!/usr/bin/env node
/**
 * Generates an animated "jet over contribution grid" SVG using latifangren's
 * GitHub contribution calendar (last 34 weeks).
 */

import fs from "node:fs";
import path from "node:path";

const USERNAME = process.env.GH_USERNAME || "latifangren";
...
async function fetchWeeks() {
  if (TOKEN) {
    try {
      ...
      console.warn("GraphQL fetch failed, falling back to public API:", err.message);
    }
  }
  ...
  try {
      ...
      if (data.contributions && Array.isArray(data.contributions)) {
        ...
        for (const item of data.contributions) {
          currentWeek.push({
            ...
            color: colorMap[item.level] || "#161b22",
          });
    ...
    console.warn("Public API fetch failed, using fallback calendar:", err.message);
  }

  return Array.from({ length: COLS }, (_, w) => ({
    contributionDays: Array.from({ length: ROWS }, (_, d) => ({
      date: `2026-01-01`,
      contributionCount: (w + d) % 4 === 0 ? 6 : (w * d) % 3,
      color: (w + d) % 4 === 0 ? "#39d353" : (w * d) % 3 ? "#0e4429" : "#161b22",
    })),
  }));
}
...
function buildJet() {
  ...
  <g transform="translate(0,0)">
    <polygon points="0,-16 8,6 4,3 -4,3 -8,6" fill="#00F3FF" stroke="#00B5E2" stroke-width="1"/>
    <polygon points="-8,6 -14,12 -4,7" fill="#00B0FF"/>
    <polygon points="8,6 14,12 4,7" fill="#00B0FF"/>
    <circle cx="0" cy="-6" r="2.2" fill="#E0F7FA"/>
    <polygon points="-3,7 3,7 0,15" fill="#FF0055">
